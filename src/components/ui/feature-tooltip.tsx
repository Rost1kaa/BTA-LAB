        ...
        createPortal(
          <AnimatePresence>
            {open && positioned && (
              ...
              >
                {/* Title */}
                <p className="text-base font-bold text-[var(--color-fg-primary)] leading-snug">
                  {name}
                </p>
                ...
                {/* Description */}
                <p className="text-base text-[var(--color-fg-secondary)] leading-relaxed">
                  {description}
                </p>
              </motion.div>
            )}
...
// ── Description Tooltip (simplified, for card descriptions) ────────────

export function DescriptionTooltip({ description, idealFor, priceExplanation }: DescriptionTooltipProps) {
  const { activeId, setActiveId } = useContext(TooltipContext);

  const idRef = useRef(
    `desc-tooltip-${Math.random().toString(36).slice(2, 8)}`,
  );
  const tooltipId = idRef.current;
          ...
          <AnimatePresence>
            {open && positioned && (
              ...
              >
                ...
                {idealFor && (
                  <>
                    <div className="mt-3 mb-2 border-t border-[var(--color-border-primary)]" />
                ...
                {priceExplanation && (
                  <>
                    {!idealFor && <div className="mt-3 mb-2 border-t border-[var(--color-border-primary)]" />}
                    <p className="text-base text-[var(--color-fg-tertiary)] leading-relaxed mt-2">
                      {priceExplanation}
                    </p>
                  </>
                )}
